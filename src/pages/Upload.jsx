  ...
  return (
    <PageTransition>
      <div className="px-4 pt-6 pb-4">
        {/* Header */}
        <div className="mb-6">
          <p className="text-foam/50 text-sm">Neuen Fang hinzufügen</p>
          <h1 className="font-display text-2xl font-extrabold text-foam">Fang loggen</h1>
        </div>
            ...
            <React.Fragment key={s}>
              <div className="flex flex-col items-center gap-1">
                <motion.div
                  ...
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
                    i < step ? 'gradient-tide text-white' :
                    i === step ? 'border-2 border-tide-400 text-tide-400' :
              ...
              {i < steps.length - 1 && (
                <div className="flex-1 h-0.5 mx-1 mb-4 rounded-full overflow-hidden bg-abyss-700">
                  <motion.div
                    className="h-full gradient-tide rounded-full"
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 0 && (
            ...
            >
              ...
              >
                {aiLoading ? (
                  <>
                    ...
                    <p className="text-tide-300 font-medium">KI-Erkennung läuft...</p>
                    <p className="text-foam/30 text-xs">Art wird identifiziert</p>
                  </>
                ) : aiDone ? (
                  <>
                    ...
                    <p className="text-tide-300 font-medium">KI-Erkennung: Hecht</p>
                    <p className="text-foam/40 text-xs">Foto hochgeladen ✓</p>
                  </>
                ) : (
                  <>
                    ...
                    <p className="text-foam font-semibold">Foto aufnehmen oder wählen</p>
                    <p className="text-foam/30 text-xs">KI erkennt die Fischart automatisch</p>
                  </>
                )}
              </motion.button>
              <motion.button
              ...
              >
                {aiDone ? 'Weiter mit KI-Vorschlag' : 'Überspringen'}
              </motion.button>
            </motion.div>
          )}
                  ...
                  <motion.button
                    ...
                    className={`p-4 rounded-2xl text-left transition-all duration-200 ${
                      selectedSpecies === s.name
                        ? 'gradient-tide glow-tide'
                        : 'glass-card border border-tide-300/10'
                    }`}
                  >
          ...
          {step === 2 && (
            ...
            >
              {[
                { label: 'Länge', value: length, setValue: setLength, unit: 'cm', min: 5, max: 200, step: 1 },
                { label: 'Gewicht', value: weight, setValue: setWeight, unit: 'kg', min: 0.1, max: 50, step: 0.1 },
              ].map((slider) => (
                <div key={slider.label} className="glass-card rounded-2xl p-5">
                  <div className="flex justify-between items-center mb-4">
                    <p className="text-foam font-bold">{slider.label}</p>
                  ...
                  <input
                    ...
                    style={{
                      background: `linear-gradient(to right, #1FA7B8 0%, #4DC3D1 ${((slider.value - slider.min) / (slider.max - slider.min)) * 100}%, rgba(10,50,68,0.8) ${((slider.value - slider.min) / (slider.max - slider.min)) * 100}%, rgba(10,50,68,0.8) 100%)`
                    }}
                  />
            ...
            >
              ...
              <div className="h-48 rounded-3xl overflow-hidden relative glass-card">
                <div className="absolute inset-0" style={{
                  background: 'radial-gradient(ellipse at 50% 50%, rgba(14,64,84,0.8) 0%, rgba(2,21,33,0.95) 100%)'
                }}>
              ...
              <div className="glass-card rounded-2xl flex items-center gap-3 px-4 py-3">
                <Search className="w-4 h-4 text-tide-400 flex-shrink-0" />
                <input
                  placeholder="Gewässer / Adresse eingeben..."
                  className="bg-transparent flex-1 text-foam placeholder-foam/30 text-sm outline-none"
                />
              </div>
