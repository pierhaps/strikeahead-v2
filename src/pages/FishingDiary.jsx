...
import { Plus, Moon, Wind, Fish } from 'lucide-react';
import PageTransition from '../components/ui/PageTransition';
import { base44 } from '@/api/base44Client';
import { Link } from 'react-router-dom';

const MOOD_EMOJI = { amazing: '🤩', great: '😄', good: '🙂', okay: '😐', tough: '😤' };
const MOOD_LABELS = { amazing: 'Fantastisch', great: 'Super', good: 'Gut', okay: 'OK', tough: 'Schwierig' };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
        ...
        <h3 className="font-display font-bold text-foam text-lg">Neuer Tagebucheintrag</h3>

        <div className="glass-card rounded-2xl p-4 flex items-center justify-between">
          <span className="text-foam/60 text-sm">Datum</span>
        ...
        <div className="glass-card rounded-2xl p-4">
          <p className="text-foam/50 text-xs mb-3">Wie war der Tag?</p>
        ...
        <textarea value={form.highlights} onChange={e => setForm(f => ({ ...f, highlights: e.target.value }))}
          placeholder="Highlights des Tages..." rows={3}
        ...
        <textarea value={form.personal_notes} onChange={e => setForm(f => ({ ...f, personal_notes: e.target.value }))}
          placeholder="Persönliche Notizen..." rows={3}
          className="w-full glass-card rounded-2xl px-4 py-3 text-foam placeholder-foam/30 text-sm outline-none border-none resize-none" />
  ...
  return (
    <PageTransition>
      <div className="px-4 pt-6 pb-24 space-y-4">
        <div>
          <p className="text-foam/50 text-sm">Dein Anglertagebuch</p>
          <h1 className="font-display text-2xl font-extrabold text-foam">Tagebuch</h1>
        </div>

        {entries.length === 0 ? (
          <div className="glass-card rounded-3xl p-10 text-center mt-8">
            <div className="text-5xl mb-4">📔</div>
            <p className="font-display font-bold text-foam text-lg">Noch keine Einträge</p>
            <p className="text-foam/40 text-sm mt-2 mb-6">Fange an deine Angelerlebnisse zu dokumentieren</p>
                ...
                <div className="p-4">
                  ...
                  {e.ai_tips?.length > 0 && (
                    <div className="mt-3 p-3 rounded-xl"
                      ...
                      <p className="text-sun-400/70 text-[10px] uppercase tracking-widest mb-1.5">KI-Tipp</p>
      ...
      <AnimatePresence>
        {showNew && <NewEntryModal onClose={() => setShowNew(false)} onSave={load} />}
      </AnimatePresence>
    </PageTransition>
  );
}