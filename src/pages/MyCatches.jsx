...
import { X, Wind, Thermometer, Anchor, Shield } from 'lucide-react';
...
const VERIFICATION_LABELS = {
  unverified: { label: 'Unverifiziert', color: 'text-foam/40' },
  photo_verified: { label: 'Foto', color: 'text-tide-400' },
  gps_verified: { label: 'GPS', color: 'text-tide-300' },
  fully_verified: { label: 'Vollständig', color: 'text-sun-400' },
};

function DetailSheet({ catch: c, onClose }) {
  if (!c) return null;
    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between mb-4">
          <div>
            <h2 className="font-display font-bold text-foam text-xl">{c.species || 'Unbekannt'}</h2>
        ...
        {(c.technique || c.bait_category) && (
          <div className="glass-card rounded-2xl p-3 mb-4">
            <p className="text-foam/40 text-xs uppercase tracking-widest mb-2">Köder & Technik</p>
        ...
        {(c.air_temp_c || c.wind_speed_kmh || c.water_temp_c) && (
          <div className="glass-card rounded-2xl p-3 mb-4">
            <p className="text-foam/40 text-xs uppercase tracking-widest mb-2">Wetterdaten</p>
            <div className="grid grid-cols-3 gap-2">
              {c.air_temp_c && <div className="text-center"><Thermometer className="w-4 h-4 text-tide-400 mx-auto mb-1" /><p className="text-foam font-bold text-sm">{c.air_temp_c}°C</p><p className="text-foam/30 text-[10px]">Luft</p></div>}
              {c.water_temp_c && <div className="text-center"><Thermometer className="w-4 h-4 text-tide-300 mx-auto mb-1" /><p className="text-foam font-bold text-sm">{c.water_temp_c}°C</p><p className="text-foam/30 text-[10px]">Wasser</p></div>}
              {c.wind_speed_kmh && <div className="text-center"><Wind className="w-4 h-4 text-foam/50 mx-auto mb-1" /><p className="text-foam font-bold text-sm">{c.wind_speed_kmh} km/h</p><p className="text-foam/30 text-[10px]">{c.wind_direction || 'Wind'}</p></div>}
            </div>
          </div>
        )}
    ...
    <PageTransition>
      <div className="px-4 pt-6 pb-4 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-foam/50 text-sm">Deine Fänge</p>
            <h1 className="font-display text-2xl font-extrabold text-foam">Meine Fänge</h1>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
          ...
          {['all','released','kept'].map(r => (
            <button key={r} onClick={() => setFilterReleased(r)}
              className={`px-3 py-2 rounded-xl text-xs font-bold flex-shrink-0 ${filterReleased === r ? 'bg-tide-500/25 text-tide-300 border border-tide-400/30' : 'glass-card text-foam/60'}`}>
              {r === 'all' ? '🐟 Alle' : r === 'released' ? '🔄 C&R' : '🍳 Behalten'}
            </button>
          ))}
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="glass-card rounded-3xl p-10 text-center mt-8">
            ...
            <p className="font-display font-bold text-foam text-lg">Keine Fänge</p>
            <p className="text-foam/40 text-sm mt-2 mb-6">Noch keine Fänge geloggt oder Filter anpassen</p>
            ...
            {filtered.map((c, i) => (
              <motion.button key={c.id} onClick={() => setSelected(c)}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-2.5">
                  <p className="text-white font-bold text-sm">{c.species || 'Unbekannt'}</p>
        ...
        <AnimatePresence>
          {selected && <DetailSheet catch={selected} onClose={() => setSelected(null)} />}
        </AnimatePresence>
