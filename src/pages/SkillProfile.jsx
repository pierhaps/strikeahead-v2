  ...
  return (
    <PageTransition>
      <div className="px-4 pt-6 pb-4 space-y-6">
        <div>
          <p className="text-foam/50 text-sm">Dein Können</p>
          <h1 className="font-display text-2xl font-extrabold text-foam">Skill-Profil</h1>
        </div>

        {skills.length === 0 ? (
          <div className="glass-card rounded-3xl p-10 text-center mt-12">
            <div className="text-5xl mb-4">🎯</div>
            <p className="font-display font-bold text-foam text-lg">Noch keine Skill-Daten</p>
            <p className="text-foam/40 text-sm mt-2 mb-6">Logge Fänge um dein Skill-Profil aufzubauen</p>
          ...
          <>
            ...
            <div className="glass-card rounded-3xl p-4">
              <p className="font-display font-bold text-foam text-sm mb-2 text-center">Skill-Radar</p>
              <ResponsiveContainer width="100%" height={280}>
                <RadarChart data={radarData} cx="50%" cy="50%" outerRadius="70%">
                  ...
                  <Radar name="Skills" dataKey="value" stroke="#1FA7B8" fill="#1FA7B8" fillOpacity={0.2} strokeWidth={2} />
                </RadarChart>
              </ResponsiveContainer>
            </div>
                    ...
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {hasGap && (
                            <span className="px-1.5 py-0.5 rounded text-[9px] font-bold"
                              style={{ background: 'rgba(245,195,75,0.15)', color: '#F5C34B' }}>Coaching empfohlen</span>
                          )}
                        </div>
                      ...
                      <div className="text-right flex-shrink-0">
                        {s.peer_percentile != null && (
                          <p className="text-tide-400 text-xs font-semibold">Top {100 - Math.round(s.peer_percentile)}%</p>
                        )}
                        {s.confidence != null && (
                          <p className="text-foam/30 text-[10px]">{s.confidence}% Konfidenz</p>
                        )}
                      </div>
                    </div>
                    {hasGap && (
                      <Link to="/coaches" className="mt-2 flex items-center gap-1 text-sun-400 text-xs font-semibold">
                        Coach finden <ChevronRight className="w-3 h-3" />
                      </Link>
                    )}
                  </motion.div>
                );
