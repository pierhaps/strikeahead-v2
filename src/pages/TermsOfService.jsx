...
const SECTION_IDS = [
  'scope','services','registration','obligations','prohibited',
  'hookpoints','subscription','liability','law','arbitration','changes',
];
...
export default function TermsOfService() {
  ...
  const sections = SECTION_IDS.map((id, i) => ({
    id,
    title: t(`terms.s${i + 1}_title`),
    content: t(`terms.s${i + 1}_content`),
  }));
    ...
    <PageTransition>
      <div className="px-4 pt-6 pb-12 max-w-2xl mx-auto space-y-4">
        <div>
          ...
          <p className="text-foam/40 text-xs mt-1">{t('terms.last_updated')}</p>
        </div>

        <div className="glass-card rounded-2xl p-4">
          <p className="text-foam/50 text-xs mb-2 font-semibold">{t('terms.toc')}</p>
